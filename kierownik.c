...
int main() {
    ...
    if (shmid == -1) {
        perror("Blad shmget w kierownik.c");
        exit(1);
    }

    sdata = (SharedData*)shmat(shmid, NULL, 0);
    if (sdata == (void*)-1) {
        perror("Blad shmat w kierownik.c");
        exit(1);
    }
    ...
    printf("[KIEROWNIK] Zarzadca uruchomiony. PID: %d. Czekam na sygnaly (SIGUSR1, SIGUSR2, SIGALRM).\n", getpid());
    ...
    if (!sdata->emergency_exit) {
        ...
        printf("[Kierownik] Godzina %02d:%02d osiagnieta. Zamykamy biletomat. Konczymy obsluge obecnych gosci.\n",
